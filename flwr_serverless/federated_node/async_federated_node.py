...
class AsyncFederatedNode:
    ...
    - [Semi-Synchronous Federated Learning for Energy-Efficient
      ...
      (https://arxiv.org/pdf/2102.02849.pdf)
    """
                ...
                if key != self.node_id:
                    ...
                    if sender_epoch is not None:
                        ...
                        aggregatable = Aggregatable(
                            ...
                            num_examples=max(
                                1, int(aggregatable.num_examples * alpha)
                            ),
    ...
    def update_parameters(
        ...
        # 1) Estado principal para agregação (como já era)
        self.model_store[self.node_id] = dict(
            aggregatable=self_aggregatable,
            model_hash=self.node_id + "_" + str(now),
            epoch=epoch,
            node_id=self.node_id,
            delay_s=metrics.get("epoch_delay_s", 0.0) if metrics else 0.0,
            published_at=now,
        )

        # 2) Novo: log estruturado por epoch/nó no SharedFolder (GCS/S3/etc.)
        try:
            log_key = f"logs/{self.node_id}/epoch_{epoch}_{int(now)}"
            self.model_store[log_key] = {
                "node_id": self.node_id,
                "epoch": epoch,
                "num_examples": num_examples,
                "metrics": metrics,
            ...
            LOGGER.warning(
                f"node {self.node_id}: failed to write log to shared folder: {e}"
            )
        ...
        LOGGER.info(
            f"node {self.node_id}: "
            f"{len(aggregatables_from_other_nodes or [])} aggregatables_from_other_nodes"
        )
        ...
        if len(aggregatables_from_other_nodes) == 0:
            ...
            parameters_from_all_nodes = [self_aggregatable] + aggregatables_from_other_nodes
